...
function getMonthIndex(month) {
  ...
  return MONTHS.findIndex((name) => name.toLowerCase() === normalized);
}

function parseFaerunDate(value) {
  if (!value) return null;
  const match = String(value).match(/^(\d{1,2})\s+([A-Za-z]+)\s+(\d{3,4})/);
  if (!match) return null;
  const month = getMonthIndex(match[2]);
  if (month < 0) return null;
  return { day: Math.min(Math.max(Number(match[1]) || 1, 1), DAYS_IN_MONTH), month, year: Number(match[3]) || DEFAULT_YEAR };
}

function addFaerunDays(date, days = 0) {
  if (!date) return null;
  const yearLength = MONTHS.length * DAYS_IN_MONTH;
  const ordinal = date.year * yearLength + date.month * DAYS_IN_MONTH + (date.day - 1) + Number(days || 0);
  const year = Math.floor(ordinal / yearLength);
  const dayOfYear = ((ordinal % yearLength) + yearLength) % yearLength;
  return { day: (dayOfYear % DAYS_IN_MONTH) + 1, month: Math.floor(dayOfYear / DAYS_IN_MONTH), year };
}

function formatFaerunDate(date) {
  if (!date || !MONTHS[date.month]) return '';
  return `${date.day} ${MONTHS[date.month]} ${date.year} рік після Потопу`;
}
...
function weatherSeed({ cycleId = '', startedAt = '' } = {}) {
  const startDate = parseFaerunDate(startedAt);
...
function generateWeatherDay({ cycleId = '', startedAt = '', dayOffset = 0 } = {}) {
  ...
  const rng = createSeededRng(weatherSeed({ cycleId, startedAt }));
  let selected = null;
  ...
  for (let offset = 0; offset <= normalizedOffset; offset += 1) {
    ...
    const pool = getWeatherPoolForSeason(seasonId);
  ...
  const seasonId = SEASON_BY_MONTH[date.month];
  if (!date || !seasonId || !selected) return null;
  ...
  return {
    ...
    icon: selected.icon,
    effects: { ...selected.effects, sumModifier: { ...selected.effects.sumModifier } }
  };
}
...
export function describeWeatherEffects(effects = {}) {
  const parts = [];
  const dc = Number(effects.dcModifier ?? 0);
  if (dc) parts.push(`DC ${dc > 0 ? '+' : ''}${dc}`);
  const sum = effects.sumModifier?.label;
  if (sum && sum !== '0') parts.push(`сума ${sum}`);
  const fishValue = Number(effects.fishValueMultiplier ?? 1);
  if (fishValue !== 1) parts.push(`ціна ${Math.round((fishValue - 1) * 100) > 0 ? '+' : ''}${Math.round((fishValue - 1) * 100)}%`);
